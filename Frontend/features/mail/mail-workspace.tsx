...
function AccountDetailCard({ account, csrfToken, onChanged }: { account: SmtpAccountRecord; csrfToken: string; onChanged: (account: SmtpAccountRecord) => Promise<void> }) {
  ...
  return (
    <Card className="h-fit xl:sticky xl:top-24">
      <CardHeader><div className="flex items-center justify-between gap-3"><CardTitle>{account.name}</CardTitle><MailStatusBadge status={account.status} /></div><CardDescription><span className="block">{t("Leave the authorization code blank to keep the stored credential unchanged.")}</span><span className="mt-1 block break-all font-mono text-[10px] text-slate-600">{account.id}</span></CardDescription></CardHeader>
...
function DeliveryDetail({ delivery }: { delivery: MailDeliveryRecord }) {
  const { t } = useLocale();
  return <Card><CardHeader><div className="flex items-center justify-between gap-3"><CardTitle>{delivery.subject}</CardTitle><MailStatusBadge status={delivery.status} /></div><CardDescription><span className="block">{delivery.clientMessageId}</span><span className="mt-1 block break-all font-mono text-[10px] text-slate-600">{delivery.id}</span></CardDescription></CardHeader><CardContent className="grid gap-3 text-sm sm:grid-cols-2"><Detail label={t("To")} value={delivery.to.join(", ")} /><Detail label={t("CC")} value={delivery.cc.join(", ") || "—"} /><Detail label={t("BCC")} value={delivery.bcc.join(", ") || "—"} /><Detail label={t("Reply-To")} value={delivery.replyTo || "—"} /><Detail label={t("Created")} value={formatDate(delivery.createdAt)} /><Detail label={t("Completed")} value={formatDate(delivery.completedAt)} /><Detail label={t("Provider message ID")} value={delivery.providerMessageId || "—"} /><Detail label={t("Error")} value={delivery.errorCode ? `${delivery.errorCode}: ${delivery.errorMessage}` : "—"} /></CardContent></Card>;
}
