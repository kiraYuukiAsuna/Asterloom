...
if [[ ! -f .env ]]; then
  ...
  printf '%s\n' \
    ...
    "ASTERLOOM_BOOTSTRAP_ADMIN_EMAIL=admin@kirayuukiasuna.cloud" \
    ...
    "ASTERLOOM_CERTIFICATE_PASSWORD=$certificate_password" > .env
  chmod 0600 .env
fi
...
install -m 0644 \
  Deploy/Nginx/asterloom.bootstrap.conf \
  /etc/nginx/sites-available/asterloom.kirayuukiasuna.cloud
ln -sfn \
  /etc/nginx/sites-available/asterloom.kirayuukiasuna.cloud \
  /etc/nginx/sites-enabled/asterloom.kirayuukiasuna.cloud
nginx -t
systemctl reload nginx

echo "Production host prerequisites prepared."
